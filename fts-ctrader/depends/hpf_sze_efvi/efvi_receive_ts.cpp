...
//***************************************************************************
void udp_quote_ts::handle_rx(struct resources* res, int pkt_buf_i, int len)
{
...
bool udp_quote_ts::init(sock_udp_param &shfe)
{
	m_udp_param			= shfe;
...
int udp_quote_ts::sl_parse(ef_filter_spec* fs, const char* ip, unsigned short port)
{
...
struct pkt_buf* udp_quote_ts::pkt_buf_from_id(struct resources* res, int pkt_buf_i)
{
	assert((unsigned) pkt_buf_i < (unsigned) res->pkt_bufs_n);
	return (pkt_buf*) ((char*) res->pkt_bufs + (size_t) pkt_buf_i * PKT_BUF_SIZE);
}

void udp_quote_ts::pkt_buf_free(struct resources* res, struct pkt_buf* pkt_buf)
{
...
void udp_quote_ts::handle_rx_discard(struct resources* res, int pkt_buf_i, int len, int discard_type)
{
...
void udp_quote_ts::refill_rx_ring(struct resources* res)
{
	struct pkt_buf* pkt_buf;
	int i;
...
void udp_quote_ts::thread_main_loop(struct resources* res)
{
	ef_event evs[32];
...
void* udp_quote_ts::func_work_thread(void* param)
{
	udp_quote_ts* ptr = (udp_quote_ts*) param;
	ptr->on_work_thread();

	return 0;
}

int udp_quote_ts::on_work_thread()
{
...
bool udp_quote_ts::bind_cpu( int cpu_id, pthread_t thd_id )
{
...
bool udp_quote_ts::start_recv_thread()
{
	pthread_t	id;
	...
	if ( 0 != pthread_create(&id, &threadAttr, udp_quote_ts::func_work_thread, (void*)(this)) )
	{
...
void udp_quote_ts::log_quote()
{
	...
	if (m_ll_exe_count > 0)
	{
		for (int i = 0; i < QT_TS_COUNT; i++)
		{
			...
			sprintf(ch_buffer, "%u,%s, %u, %lld, %lld, %lld, %.2f, %lld, %d\n",
			        m_exe[i].m_seq,	
                                m_exe[i].m_symbol,

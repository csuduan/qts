...
public class TdSpi extends CThostFtdcTraderSpi {
	...
	public void OnRspOrderInsert(CThostFtdcInputOrderField pInputOrder, CThostFtdcRspInfoField pRspInfo, int nRequestID,
		...
		if(this.orderMap.containsKey(orderRef)){
			...
			ctpGateway.emitEvent(FastEvent.EVENT_ORDER,order);
			// 发送委托事件
	...
	public void OnRspQryInvestorPosition(CThostFtdcInvestorPositionField pInvestorPosition,
		...
		if (positionMap.containsKey(posName)) {
			...
			position.setDirection(
					CtpConstant.posiDirectionMapReverse.getOrDefault(pInvestorPosition.getPosiDirection(), POS_DIRECTION.NONE));
		...
		if (bIsLast) {
			for (Position tmpPosition : positionMap.values()) {
				...
				// 发送持仓事件
				ctpGateway.emitEvent(FastEvent.EVENT_POSITION,tmpPosition);
	...
	public void OnRspQryTradingAccount(CThostFtdcTradingAccountField pTradingAccount, CThostFtdcRspInfoField pRspInfo,
		...
		ctpGateway.emitEvent(FastEvent.EVENT_ACCT,account);
	...
	public void OnRspQryInstrument(CThostFtdcInstrumentField pInstrument, CThostFtdcRspInfoField pRspInfo,
		...
		ctpGateway.emitEvent(FastEvent.EVENT_CONTRACT,contract);
...
//		int sessionID = pOrder.getSessionID();
	...
	public void OnRtnTrade(CThostFtdcTradeField pTrade) {
		...
		trade.setDirection(CtpConstant.directionMapReverse.getOrDefault(pTrade.getDirection(), TRADE_DIRECTION.NONE));
		trade.setOffset(CtpConstant.offsetMapReverse.getOrDefault(pTrade.getOffsetFlag(), OFFSET.NONE));
		...
		ctpGateway.emitEvent(FastEvent.EVENT_TRADE,trade);
	}

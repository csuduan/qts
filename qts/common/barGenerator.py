...
class BarGenerator:
    ...
    def update_tick(self, tick: TickData) -> None:
        ...
        if not self.bar:
            ...
            (self.bar.time.minute != tick.time.minute)
            or (self.bar.time.hour != tick.time.hour)
        ):
            self.bar.time = self.bar.time.replace(
    ...
    def update_bar_hour_window(self, bar: BarData) -> None:
        ...
        # If minute bar of new hour, then push existing window bar
            ...
            dt = bar.time.replace(minute=0, second=0, microsecond=0)
            self.hour_bar = BarData(
                symbol=bar.symbol,
                exchange=bar.exchange,
                datetime=dt,
    ...
    def update_bar_daily_window(self, bar: BarData) -> None:
        ...
        if bar.time.time() == self.daily_end:
            self.daily_bar.time = bar.time.replace(
                hour=0,
                minute=0,
                second=0,
                microsecond=0
